...
if __name__ == '__main__': 
    ...
    # Read num_classes from config.json
    with open('src/config.json') as f:
        config = json.load(f)
    num_classes = len(config['classes'])

    model = DETR(num_classes=num_classes)
    # NOTE: Skipping model.load_pretrained('pretrained/4426_model.pt')
    # The Git LFS file is corrupted/missing. Training from scratch using
    # the ImageNet-pretrained ResNet-50 backbone instead.
